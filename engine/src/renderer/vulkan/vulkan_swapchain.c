    ...
    vulkan_swapchain* swapchain)
{
    ...
    swapchain->max_frames_in_flight = 2;    // triple buffer swapchain.
    ...
    for (u32 i = 0; i < context->device.swapchain_support.present_mode_count; ++i) {
        ...
        // VK_PRESENT_MODE_MAILBOX_KHR, low latency, always choose the newest frame.
    ...
    vulkan_image_create(
        ...
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,        // use gpu memory not shared memory.
        TRUE,
